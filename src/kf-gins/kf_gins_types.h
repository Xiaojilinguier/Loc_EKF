...
typedef struct ImuNoise {
    Eigen::Vector3d gyr_arw;      // 陀螺仪角度随机游走
    Eigen::Vector3d acc_vrw;      // 加计随机游走
    Eigen::Vector3d gyrbias_std;  // 陀螺仪偏置标准差
    Eigen::Vector3d accbias_std;  // 加计偏置标准差
    Eigen::Vector3d gyrscale_std; // 陀螺仪比例因子误差标准差
    Eigen::Vector3d accscale_std; // 加速度计比例因子误差标准差
    double corr_time;             // 相关时间常数
} ImuNoise;
